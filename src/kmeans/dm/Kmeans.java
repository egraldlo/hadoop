...
public class Kmeans extends Configured implements Tool{
    ...
    public static boolean stopIteration(Configuration conf) throws IOException{
        ...
        //前面的质心文件
        Path pervCenterFile=new Path("/home/casa/input/centers");
        //现在的质心文件
        Path currentCenterFile=new Path("/home/casa/output/part-r-00000");
        
        if(!(fs.exists(pervCenterFile) && fs.exists(currentCenterFile))){
            log.info("两个质心文件需要同时存在"+(fs.exists(pervCenterFile)==true)+(fs.exists(currentCenterFile)==true));
            System.exit(1);
        }
        ...
        log.info("check br1: "+(br1.readLine()!=null)+" check br2: "+(br2.readLine()!=null));
        while((line1=br1.readLine())!=null && (line2=br2.readLine())!=null){
            ...
            log.info("dist "+Sample.getEulerDist(prevCenter, currCenter));
        ...
        log.info("rename successful, stop: "+(stop==true));
        return stop;
    }
     
    //map函数计算出每个节点所属的中点号，然后输出<中点号，本节点>
    public static class ClusterMapper extends Mapper<LongWritable, Text, IntWritable, Sample> {
        ...
        public void cleanup(Context context) throws IOException,InterruptedException {
            ...
            log.info("in the map func and in cleanup");
    ...
    //reduce计算出每个中点编号的真正质心输出<质心编号，质心>
    public static class UpdateCenterReducer extends Reducer<IntWritable, Sample, IntWritable, Sample> {
        ...
        public void cleanup(Context context) throws IOException,InterruptedException{
            for(int i=0;i<center.arr.length;i++)
                center.arr[i]/=count;
            log.info("in the reduce and and in the cleanup func!");
            context.write(new IntWritable(prev), center);
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        //质心文件每行的第一个数字是索引
        FileInputFormat.setInputPaths(job, "/home/casa/input/centers");
        Path outDir=new Path("/home/casa/output/");
...
//        job.setOutputFormatClass(FileInputFormat.class);
        log.info("4");
    ...
    public static void main(String[] args) throws Exception {
        ...
        //样本数据文件中每个样本不需要标记索引
        Path dataFile=new Path("/home/casa/data/kmeans/input/apache");
        ...
        do {
            ...
            log.info("iteration "+iteration+" end "+success+" true? "+(stopIteration(conf)==true));
        } while (success == 1 && iteration++ < MAXITERATIONS
                && (!stopIteration(conf)));
        log.info("Success.Iteration=" + iteration);
         
        //迭代完成后再执行一次mapper，输出每个样本点所属的分类--在/home/casa/output2/part-m-00000中
        //质心文件保存在/home/casa/input/centers中
        Job job=new Job(conf);
        job.setJarByClass(Kmeans.class);
         
        FileInputFormat.setInputPaths(job, "/home/casa/input/centers");
        Path outDir=new Path("/home/casa/output2/");
        fs.delete(outDir,true);
        FileOutputFormat.setOutputPath(job, outDir);
